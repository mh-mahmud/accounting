	...
	*/
	...
	public function create_user($data=null){
		...
		if( $input->user_id ){
			#update query
			$sql = "
			UPDATE user SET COMPANY_ID=?, USER_NAME=?, USER_EMAIL=?, USER_PHONE=?, ROLE_ID=?,n UPDATED_BY=?, UPDATED_DATE=? 
			WHERE USER_ID=?";
			$this->db->query($sql, array($company_id, $input->user_name, $input->user_email, $input->user_phone, $input->user_role, $this->webspice->get_user_id(), $this->webspice->now(), $input->user_id)); 
		...
		$sql = "
		INSERT INTO user
		(COMPANY_ID, USER_NAME, USER_EMAIL, USER_PHONE, USER_PASSWORD, ROLE_ID, CREATED_BY, CREATED_DATE, STATUS)
		VALUES
		(?, ?, ?, ?, ?, ?, ?, ?, 7)";
		$this->db->query($sql, array($company_id, $input->user_name, $input->user_email, $input->user_phone,
	...
	public function create_role($data=null) {
