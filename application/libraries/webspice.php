...
class Webspice{
	...
	function settings(){
		...
		$settings->site_title = 'Base4 Accounting System';
		$settings->domain_name = 'Base4 Accounting System';
		$settings->site_slogan = '';
		$settings->site_url = 'http://localhost/accounting';
		$settings->site_url_prefix = '/accounting/';
	...
	*/
	...
	function company_name($id) {
		...
		$sql = $CI->db->query("SELECT COMPANY_NAME FROM company WHERE COMPANY_ID=".$id);
		$company = $sql->row()->COMPANY_NAME;
		return $company;
	}
	...
	public function late_counter($counter, array $data) {
		for($i=1; $i<=$counter; $i++) {
			if(in_array($i, $data)) {
				echo "<td>Bubu</td>";
			}
	...
	public function class_name($class_id=null) {
		$CI = &get_instance();
		$CI->load->database();

		if($class_id) {
			$class_name = $CI->db->query("SELECT CLASS_NAME FROM class WHERE CLASS_ID=".$class_id)->row()->CLASS_NAME;
			return $class_name;
		}
		else {
			return false;
	...
	public function section_name($section_id=null) {
		$CI = &get_instance();
		$CI->load->database();

		if($section_id) {
			$section_name = $CI->db->query("SELECT SECTION_NAME FROM section WHERE SECTION_ID=".$section_id)->row()->SECTION_NAME;
			return $section_name;
		}
		else {
			return false;
	...
	public function student_name($student_id=null) {
		$CI = &get_instance();
		$CI->load->database();

		if($student_id) {
			$student_name = $CI->db->query("SELECT si.NAME FROM student_info AS si INNER JOIN student_data AS sd ON sd.STUDENT_ID=si.STUDENT_ID WHERE sd.STUDENT_DATA_ID=".$student_id)->row()->NAME;
			return $student_name;
		}
		else {
			return false;
	...
	public function teacher_name($teacher_id=null) {
		$CI = &get_instance();
		$CI->load->database();

		if($teacher_id) {
			$teacher_name = $CI->db->query("SELECT TEACHER_NAME FROM teacher WHERE TEACHER_ID=".$teacher_id)->row()->TEACHER_NAME;
			return $teacher_name;
		}
		else {
			return false;
	...
	public function account_head_name($head_id=null) {
		$CI = &get_instance();
		$CI->load->database();

		if($head_id) {
			$head_name = $CI->db->query("SELECT ACC_NAME FROM account_name WHERE CODE=".$head_id)->row()->ACC_NAME;
			return $head_name;
		}
		else {
			return false;
	...
	public function account_head_by_id($head_name) {
		$CI = &get_instance();
		$CI->load->database();

		if($head_name) {
			$head_id = $CI->db->query("SELECT ACC_ID, CODE FROM account_name WHERE ACC_NAME='$head_name'")->row()->CODE;
			return $head_id;
		}
		else {
			return false;
	...
	/************************************
	*******badge id per date basis*******
	************************************/
	/*public function find_acc_badge_id() {
		$CI = &get_instance();
		$CI->load->database();

		$badge_id = $CI->db->query("SELECT ACC_BADGE_ID FROM account_transaction WHERE CREATED_DATE='".date('Y-m-d')."' ORDER BY ACC_BADGE_ID DESC LIMIT 1")->row();
		if(count($badge_id)) {
			$badge_id = $badge_id->ACC_BADGE_ID + 1;
		}
		else {
			$badge_id = 1;
		}
	...
	public function find_acc_badge_id() {
		...
		$badge_id = $CI->db->query("SELECT ACC_BADGE_ID FROM account_transaction ORDER BY ACC_BADGE_ID DESC LIMIT 1")->row();
		if(count($badge_id)) {
			$badge_id = $badge_id->ACC_BADGE_ID + 1;
		}
		else {
	...
	********************/
	...
	public function class_wise_admission_fee($class_id) {
		$ci = &get_instance();
		$ci->load->database();

		$admission_fee = $ci->db->query("SELECT AMOUNT FROM payment_category WHERE CATEGORY_NAME LIKE '%Admission Fee%' AND CLASS_ID='".$class_id."'")->row()->AMOUNT;
		dd($admission_fee);
	}

	public function class_wise_tuition_fee($class_id) {
		$ci = &get_instance();
		$ci->load->database();

		$tuition_fee = $ci->db->query("SELECT AMOUNT FROM payment_category WHERE CATEGORY_NAME LIKE '%Tuition Fee%' AND CLASS_ID='".$class_id."'")->row()->AMOUNT;
		return $tuition_fee;
	}

	public function student_wise_account_head_id($student_id) {
		$ci = &get_instance();
		$ci->load->database();

		$head_id = $ci->db->query("SELECT ACC_ID, CODE FROM account_name WHERE ACC_NAME LIKE '%Tuition Fees & others student fees%' AND STUDENT_ID={$student_id}")->row()->CODE;
		return $head_id;

	}

	public function teacher_wise_account_head_id($teacher_id) {
		$ci = &get_instance();
		$ci->load->database();

		$head_id = $ci->db->query("SELECT ACC_ID, CODE FROM account_name WHERE ACC_NAME LIKE '%Salary Payable%' AND TEACHER_ID={$teacher_id}")->row()->CODE;
		return $head_id;
	}

	public function balance_head_type($acc_code=null, $type="code") {
		$ci = &get_instance();
		$ci->load->database();

		if($acc_code) {
			if($type == "code") {
				$data = $ci->db->query("SELECT ACC_HEAD_TYPE FROM account_name WHERE CODE='".$acc_code."'")->row()->ACC_HEAD_TYPE;
			}
			else if($type == "name") {
				$data = $ci->db->query("SELECT ACC_HEAD_TYPE FROM account_name WHERE ACC_NAME='".$acc_code."'")->row()->ACC_HEAD_TYPE;
			}
	...
	public function acc_group_head($acc_code=null, $type="code") {
		$ci = &get_instance();
		$ci->load->database();

		if($acc_code) {
			if($type == "code") {
				$data = $ci->db->query("SELECT GROUP_HEAD_NAME FROM account_name WHERE CODE='".$acc_code."'")->row()->GROUP_HEAD_NAME;
			}
			else if($type == "name") {
				$data = $ci->db->query("SELECT GROUP_HEAD_NAME FROM account_name WHERE ACC_NAME='".$acc_code."'")->row()->GROUP_HEAD_NAME;
			}
	...
	public function account_b($acc_id=null) {
		
		$CI = &get_instance();
		$CI->load->database();
        $acc_amount = 0;       				   
		
			$debit = $CI->db->query("SELECT SUM(DEBIT)AS D FROM general_ledger WHERE ACC_CODE=".$acc_id)->row();
			$credit = $CI->db->query("SELECT SUM(CREDIT)AS C FROM general_ledger WHERE ACC_CODE=".$acc_id)->row();
			$balance_type = $CI->db->query("SELECT OPENING_BALANCE_TYPE FROM account_name WHERE CODE=".$acc_id)->row();

			if($balance_type->OPENING_BALANCE_TYPE=='Debit'){

				$acc_amount=($debit->D-$credit->C);
			}
			else{
				$acc_amount=($credit->C-$debit->D);
			}
			return $acc_amount;
	}
